...
/* Modified by (c) Edet Noah 2021                                                    MIT Licence  */
/*                                                                                               */
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
...
Aes.cipher = (input, w) => {
  ...
  state = Aes.subBytes(state, Nb);
  state = Aes.shiftRows(state, Nb);
  state = Aes.mixColumns(state, Nb); // ADDED MIXCOLUMNS
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
if (typeof module != "undefined" && module.exports) module.exports = Aes; // ≡ export default noahAes
...
if (typeof module != "undefined" && module.exports)
  var Aes = require("./noahAes.js"); // ≡ import Aes from 'noahAes.js'
...
Aes.Ctr.encrypt = (plaintext, password, nBits) => {
  ...
  var nonce = new Date().getTime(); // timestamp: milliseconds since 1-Jan-2021
...
// var encr = noahAes.Ctr.encrypt('big secret you dont have to know', 'pāşšŵōřđ', 256);
// console.log(encr);
 ...
 *
 * @example
 *   var decr = noahAes.Ctr.decrypt('lwGl66VVwVObKIr6of8HVqJr', 'pāşšŵōřđ', 256); // 'big secret'
 */
...
Aes.Ctr.decrypt = (ciphertext, password, nBits) => {
  ...
  plaintext = plaintext; // decode from UTF8 back to Unicode multi-byte chars
